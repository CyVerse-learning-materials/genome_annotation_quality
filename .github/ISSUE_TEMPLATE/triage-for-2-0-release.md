...
### 1. Check on file versions (files should have a version comment, usually in the first line of the file)
...
### 2. Check version and release are at least 2.0 in the following files

- [ ] `misc/cyverse_spinx_conf.py`
- [ ] `conf.py`

### 3. Check on the following required formatting for all pages
...
- [ ] All hyperlinks in documentation are on the repo's `custom_urls.txt` or
  ...
  **Note**:  We want to avoid:
    ...
    -  Where possible links should NOT be on the `.rst` page but on a single
        document that is included. (e.g. `custom_urls.txt` or
        `cyverse_rst_defined_substitutions.txt`)
...
- [ ] The Editor has checked the tutorial functions (links work, apps, work,
      etc.)
